...
export const FilterState = ({ jobApplications }: { jobApplications: JobApplication[] }) => {
  const applied = jobApplications.filter((job: JobApplication) => job.status === "applied");
  const pending = jobApplications.filter((job: JobApplication) => job.status === "pending");
  const accepted = jobApplications.filter((job: JobApplication) => job.status === "accepted");
  const rejected = jobApplications.filter((job: JobApplication) => job.status === "rejected");
